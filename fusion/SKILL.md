...
| 公開URL | https://pasarinobuhito0409-droid.github.io/trip-shioris/fusion/ |
| データ保存 | localStorage（key=`fusion_textbook_v1`） |

## ★ チェック項目の「📖 やり方」運用ルール（重要）★

各チェック項目は **ゴール（○○できる）＋ 📖やり方（具体手順 steps[]）** の2層構造。
三輪さんはADHD特性で「抽象的なゴールだけ」だと動けない（[[feedback_assembly_thinking]]：抽象NG・手順まで分解）。

| ルール | 内容 |
|---|---|
| データ構造 | `["A1", "ラベル", ["手順1", "手順2", ...]]`（3要素目が手順配列） |
| 表示 | `<details>` でタップ開閉。チェック（□）とやり方（青ボタン）は分離 |
| 手順の書き方 | やさしい日本語・番号で1ステップずつ・キー操作は `<b>L</b>` で太字 |
| **追加タイミング** | **Phaseに進んだ時に、そのPhaseだけ手順を入れる**（先走り禁止＝古くなるリスク回避・2026-05-31三輪さん確定） |
| 現状 | Phase 0 のみ手順入り済み。Phase 1〜6 は項目名だけ（進む時に追加） |
